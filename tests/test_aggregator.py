"""
Unit tests for Blockchain Aggregator Smart Contract

This module contains tests for the Aggregator.sol smart contract used in the
MedRAG blockchain-enabled federated learning system. Tests verify:
- Contract compilation and deployment
- Participant management (adding/removing)
- Weight update functionality
- Aggregation logic
- Access control and security

The tests use PyEVM backend for local testing without requiring a real blockchain.
"""
...
# Find the repository root by looking for the tests directory
repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))

if sys.platform == 'win32':
    SOLC_BINARY_PATH = os.path.join(repo_root, "tests", "solc-0.8.23-win32", "solc.exe")
elif sys.platform == 'darwin':
    SOLC_BINARY_PATH = os.path.join(repo_root, "tests", "solc-0.8.23-macos", "solc-macos")
elif sys.platform == 'linux':
    SOLC_BINARY_PATH = os.path.join(repo_root, "tests", "solc-0.8.23-linux", "solc-static-linux")
else:
